...
- [Luồng nghiệp vụ order](#luồng-nghiệp-vụ-order)
- [Trạng thái nghiệp vụ](#trạng-thái-nghiệp-vụ)
- [MQTT](#mqtt)
- [Tài liệu API](#tài-liệu-api)
- [Frontend](#frontend)
- [AI microservice](#ai-microservice)
- [Firmware ESP32](#firmware-esp32)
- [Dữ liệu test cục bộ](#dữ-liệu-test-cục-bộ)
- [Kiểm thử](#kiểm-thử)
...
10. Nếu MQTT được bật, backend gửi command `UNLOCK` cho từng locker xuống ESP32.

### Xác nhận gửi đồ

1. ESP32 nhận `UNLOCK` và mở đúng locker, không lưu trạng thái order.
2. ESP32 luôn theo dõi reed switch của tất cả locker.
3. Khi cửa đóng ổn định đủ 3 giây, ESP32 khóa solenoid và publish `LOCK`.
4. Backend tìm `OrderLocker` đang `WAIT_FOR_DEPOSIT` theo `stationId + lockerId`.
5. Nếu tìm thấy, backend chuyển `OrderLocker` sang `WAIT_FOR_COLLECTION`; nếu không có order phù hợp thì bỏ qua `LOCK`.
...
### Backend mở locker sau khi tạo order
...
### ESP32 xác nhận đã đóng và khóa locker
...
Backend lấy `stationId` và `deviceId` từ topic. Một tín hiệu `LOCK` không có
`OrderLocker` đang `WAIT_FOR_DEPOSIT` tương ứng vẫn được nhận hợp lệ nhưng không
làm thay đổi order nào.
...
### Chạy nhận diện trên Raspberry Pi

File client nằm tại `HardwareCode/raspberry_face_auth.py`. Cài dependency và cấu hình UUID của bản ghi Raspberry Pi trong bảng `devices`:

```bash
python3 -m pip install -r HardwareCode/raspberry_requirements.txt
python3 HardwareCode/raspberry_face_auth.py
```

Device test được hardcode trong script với UUID:

```text
b3e80147-a2f4-4d77-8b90-18e6d522d202
```
...
Có thể đổi bằng biến môi trường `SMARTBOX_FACE_AUTH_URL`. Client dùng model InsightFace `buffalo_l`, chuẩn hóa L2 và chỉ gửi vector 512 chiều; ảnh khuôn mặt không được gửi lên backend.
...
3. Kiểm tra command `UNLOCK`.
4. Publish `LOCK` với đúng `lockerId` vào event topic trong vòng timeout.
